...
class NETFLIX5(GeneralRecommender):
    def __init__(self, config, dataset):
        super(NETFLIX5, self).__init__(config, dataset)
        self.sparse = True
        ...
        self.multihead_attention = nn.MultiheadAttention(embed_dim=self.embedding_dim * 2, num_heads=8, batch_first=True)
        ...
        self.d = config['d']
        self.hyper_adj = config['hyper_adj']
    ...
    def pre_epoch_processing(self):
        ...
        if self.hyper_adj == 'masked':
            # masked_adj 是一个稀疏张量，需要转换为稠密形式进行切片
            dense_adj = self.masked_adj.to_dense()
            # 提取子矩阵 [ :self.n_users, self.n_users: ]
            self.masked_R = dense_adj[:self.n_users, self.n_users:]
            # 如果需要保留 masked_R 为稀疏形式，可以转换回稀疏张量
            self.masked_R = self.masked_R.to_sparse()
    ...
    def forward(self, adj, train=False):
        ...
        final_h_image_embeds = all_h_image_embeds 
        final_h_text_embeds = all_h_text_embeds 
        ...
        if self.agg == "concat":
            ...
            all_embeddings_users = torch.cat(
                [final_id_preference, final_image_preference, final_text_preference, final_h_user_embeds], dim=1  # fused user
            ...
            all_embeddings_items = final_id_emb + final_image_emb + final_text_emb + final_h_item_embeds # [# of items, feat_embed_dim]
