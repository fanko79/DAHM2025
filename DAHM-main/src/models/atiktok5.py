...
class ATIKTOK5(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ATIKTOK5, self).__init__(config, dataset)
        self.sparse = True
        ...
        # audio_adj_file = os.path.join(self.dataset_path, 'audio_adj_{}_{}.pt'.format(self.knn_k, self.sparse))
        ...
        self.audio_preference = nn.Embedding(self.n_users, self.embedding_dim)
        nn.init.xavier_uniform_(self.image_preference.weight)
        nn.init.xavier_uniform_(self.text_preference.weight)
        nn.init.xavier_uniform_(self.audio_preference.weight)
    ...
    def pre_epoch_processing(self):
        ...
        if self.pruning_random:
            ...
            if self.d:
                ...
                values_file = os.path.join(self.dataset_path, 'd_values.txt')
                def save_values_to_file(values_list, filename):
                    with open(filename, 'w') as file:
                        for value in values_list:
                            file.write(f"{value}\n")
                if not os.path.exists(values_file):
                    save_values_to_file(self.edge_values.tolist(), values_file)
    ...
    def forward(self, adj, train=False):
        ...
        if self.a_feat is not None:
            audio_feats = self.audio_trs(self.audio_embedding.weight) # I,dm → I,64

        # hyperedge dependencies constructing
        if self.v_feat is not None:
            iv_hyper = torch.mm(self.image_embedding.weight, self.v_hyper) # I, dm → I, hyper_num
            uv_hyper = torch.mm(self.R, iv_hyper) # useless
            iv_hyper = F.gumbel_softmax(iv_hyper, self.tau, dim=1, hard=False)
            uv_hyper = F.gumbel_softmax(uv_hyper, self.tau, dim=1, hard=False) # useless
        if self.t_feat is not None:
            it_hyper = torch.mm(self.text_embedding.weight, self.t_hyper)
            ut_hyper = torch.mm(self.R, it_hyper) # useless
            it_hyper = F.gumbel_softmax(it_hyper, self.tau, dim=1, hard=False)
            ut_hyper = F.gumbel_softmax(ut_hyper, self.tau, dim=1, hard=False) # useless
        if self.a_feat is not None:
            ia_hyper = torch.mm(self.audio_embedding.weight, self.a_hyper)
            ua_hyper = torch.mm(self.R, ia_hyper) # useless
            ia_hyper = F.gumbel_softmax(ia_hyper, self.tau, dim=1, hard=False)
            ua_hyper = F.gumbel_softmax(ua_hyper, self.tau, dim=1, hard=False) # useless

        image_preference = self.image_preference.weight  # [U, 64]
        text_preference = self.text_preference.weight
        audio_preference = self.audio_preference.weight
        ...
        if self.purifer == 'ori':
            ...
            audio_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_t(audio_feats))
        ...
        ego_audio_emb = torch.cat([audio_preference, audio_item_embeds], dim=0)  # [U+I-img, d]

        all_image_emb = [ego_image_emb]
        all_text_emb = [ego_text_emb]
        all_audio_emb = [ego_audio_emb]
        for layer in range(self.n_m_layers):
            side_image_emb = torch.sparse.mm(adj, ego_image_emb)
            if self.lsim == "sim":
                _weights = F.cosine_similarity(side_image_emb, all_image_emb[0], dim=-1)
                side_image_emb = torch.einsum('a,ab->ab', _weights, side_image_emb)
            ego_image_emb = side_image_emb
            all_image_emb += [ego_image_emb]

            side_text_emb = torch.sparse.mm(adj, ego_text_emb)
            if self.lsim == "sim":
                _weights = F.cosine_similarity(side_text_emb, all_text_emb[0], dim=-1)
                side_text_emb = torch.einsum('a,ab->ab', _weights, side_text_emb)
            ego_text_emb = side_text_emb
            all_text_emb += [ego_text_emb]

            side_audio_emb = torch.sparse.mm(adj, ego_audio_emb)
            if self.lsim == "sim":
                _weights = F.cosine_similarity(side_audio_emb, all_audio_emb[0], dim=-1)
                side_audio_emb = torch.einsum('a,ab->ab', _weights, side_audio_emb)
            ego_audio_emb = side_audio_emb
            all_audio_emb += [ego_audio_emb]
        ...
        if self.m_agg == "last":
            ...
            final_audio_preference, final_audio_emb = torch.split(
                ego_audio_emb, [self.n_users, self.n_items], dim=0
            )
            ...
            mean_all_audio_emb = torch.mean(torch.stack(all_audio_emb), dim=0)
            final_audio_preference, final_audio_emb = torch.split(mean_all_audio_emb, [self.n_users, self.n_items], dim=0)
        ...
        for i in range(self.n_hyper_layers):
            ...
            lat = self.drop(ia_hyper)  # I, hyper_num
            hyper_audio_item_embeds = torch.mm(lat.T, final_id_emb)  # hyper_num, 64
            hyper_audio_item_embeds = torch.mm(lat, hyper_audio_item_embeds)
        ...
        hyper_audio_user_embeds = torch.sparse.mm(self.R, hyper_audio_item_embeds)
        hyper_audio_embeds = torch.cat([hyper_audio_user_embeds, hyper_audio_item_embeds], dim=0)
        # ghe_embs = hyper_image_embeds + hyper_text_embeds

        att_common = torch.cat([self.query_common(hyper_image_embeds), self.query_common(hyper_text_embeds), self.query_common(hyper_audio_embeds)], dim=-1)
        weight_common = self.softmax(att_common)
        common_embeds = weight_common[:, 0].unsqueeze(dim=1) * hyper_image_embeds + weight_common[:, 1].unsqueeze(
            dim=1) * hyper_text_embeds + weight_common[:, 2].unsqueeze(dim=1) * hyper_audio_embeds # [U+I, d]
        common_embeds, noise_embeds = self.denoise(common_embeds, content_embeds)
        all_h_image_embeds = hyper_image_embeds - common_embeds
        all_h_text_embeds = hyper_text_embeds - common_embeds
        all_h_audio_embeds = hyper_audio_embeds - common_embeds

        #image_prefer = self.gate_image_prefer(content_embeds) # U+I, d
        ...
        final_h_audio_embeds = all_h_audio_embeds # torch.multiply(audio_prefer, all_h_audio_embeds)

        final_hyper_embeds = (1 / 4.0) * common_embeds + (1 / 4.0) * final_h_image_embeds + (1 / 4.0) * final_h_text_embeds + (1 / 4.0) * final_h_audio_embeds# Emul
        ...
        if self.agg == "concat":
            all_embeddings_items = torch.cat(
                [final_id_emb, final_image_emb, final_text_emb, final_audio_emb, final_h_item_embeds], dim=1  # fused item
            )  # [# of items, feat_embed_dim * 2]
            all_embeddings_users = torch.cat(
                [final_id_preference, final_image_preference, final_text_preference, final_audio_preference, final_h_user_embeds], dim=1  # fused user
            ...
            all_embeddings_items = final_id_emb + final_image_emb + final_text_emb + final_audio_emb + final_h_item_embeds # [# of items, feat_embed_dim]
            all_embeddings_users = (
                    final_id_preference + final_image_preference + final_text_preference + final_audio_preference + final_h_user_embeds
            )  # [# of users, feat_embed_dim]

        if train:
            return all_embeddings_users, all_embeddings_items, final_id_preference, final_id_emb, final_image_preference, final_text_preference, final_audio_preference, \
                   final_image_emb, final_text_emb, final_audio_emb, hyper_image_user_embeds, hyper_text_user_embeds, hyper_audio_user_embeds, \
                   hyper_image_item_embeds, hyper_text_item_embeds, hyper_audio_item_embeds
    ...
    def calculate_loss(self, interaction):
        ...
        ua_embeddings, ia_embeddings, final_id_preference, final_id_emb, final_image_preference, final_text_preference, final_audio_preference,\
        final_image_emb, final_text_emb, final_audio_emb,hyper_image_user_embeds, hyper_text_user_embeds, hyper_audio_user_embeds,\
        hyper_image_item_embeds, hyper_text_item_embeds, hyper_audio_item_embeds = self.forward(self.forward_adj, train=True)
        ...
        if self.cl_loss_type == "InfoNCE":
            ...
            cl_loss = self.batched_contrastive_loss(final_image_preference[users], u_g_embeddings) + self.batched_contrastive_loss(
                final_text_preference[users], u_g_embeddings) + self.batched_contrastive_loss(final_audio_preference[users], u_g_embeddings)
            cl_loss = self.cl_loss * cl_loss
            if self.cl_loss2 != 0.0:
                local_user = final_id_preference+final_image_preference+final_text_preference+final_audio_preference
                cl_loss2 = self.batched_contrastive_loss(hyper_image_user_embeds[users], local_user[users]) + self.batched_contrastive_loss(
                hyper_text_user_embeds[users], local_user[users]) + self.batched_contrastive_loss(hyper_audio_user_embeds[users], local_user[users])
